---
title: "INFSCI 2595 Fall 2025 Homework: 08"
subtitle: "Assigned November 6, 2025; Due: November 12, 2025"
author: "Surya Tej Boddu"
date: "Submission time: November 12, 2025 at 11:59PM EST"
...
ggplot(df, aes(x = x2, y = y)) +
  ...
  theme_bw()
```
* The scatter plot of y vs x1 does not show a clear linear trend. The points appear fairly scattered across the range of x1 with no obvious upward or downward pattern, suggesting little to no strong linear relationship between x1 and y.
* The scatter plot of y vs x2 shows a slight inverted U-shape pattern. The y values tend to be higher around x2 = 0 and lower at the extremes (both negative and positive ends). This suggests a possible non-linear (quadratic) relationship between x2 and y, though the signal is noisy.
* Overall, neither input shows a strong linear trend with the response, but x2 hints at a weak nonlinear relationship.
...
summary(mod01)
```
* Both confidence intervals cross zero, suggesting weak significance.
* x1 has a p-value of 0.0495, which is just below 0.05, so it is marginally statistically significant
* x2 has a p-value of 0.0960, which is above 0.05, so it is not statistically significant at the 5% level (though marginal)
* The overall model F-statistic p-value is 0.05118, which is just above 0.05, meaning the model as a whole is borderline significant.
* The R-squared is very low at 0.0595, indicating that x1 and x2 together explain only about 6% of the variance in y, so this simple linear additive model is a poor fit.
...
summary(mod02)
```
* The model contains 3 features (plus the intercept): x1, x2, and the interaction term x1:x2.
* All three features are statistically significant at the 5% level, with p-values of 0.0203, 0.0495, and 0.0180 respectively. This is also confirmed by the coefplot, where x1's confidence interval does not cross zero.
* The R-squared improved to 0.113 compared to mod01's 0.0595, suggesting that adding the interaction term helped explain more variance in y. The overall model p-value of 0.009 indicates the model as a whole is statistically significant.
...
summary(mod03)
```
* The model contains 8 features (plus the intercept): x1, I(x1^2), x2, I(x2^2), x1:x2, x1:I(x2^2), I(x1^2):x2, and I(x1^2):I(x2^2).
* Only two features are statistically significant. The intercept has a p-value of 5.13e-07 and I(x2^2) has a p-value of 3.21e-10, both highly significant. All other features, including the interaction terms, have p-values above 0.05 and are not statistically significant.
...
summary(mod04)
```
None of the individual features are statistically significant. All p-values are well above 0.05 and all confidence intervals cross zero. This is likely due to overfitting and multicollinearity among the many polynomial and interaction terms.
...
summary(mod05)
```
None of the individual features are statistically significant. All p-values are well above 0.05 and all confidence intervals cross zero. The large number of spline and interaction terms leads to high multicollinearity and very wide confidence intervals.
...
summary(mod06)
```
None of the individual features are statistically significant. All p-values are well above 0.05 and all confidence intervals cross zero. The model has 60 features (plus intercept), and the extremely large number of spline and interaction terms causes huge multicollinearity. This resulted in very wide confidence intervals.
...
model_comparison <- bind_rows(
  glance(mod01) %>% mutate(model = "mod01"),
  glance(mod02) %>% mutate(model = "mod02"),
  glance(mod03) %>% mutate(model = "mod03"),
  glance(mod04) %>% mutate(model = "mod04"),
  glance(mod05) %>% mutate(model = "mod05"),
  glance(mod06) %>% mutate(model = "mod06")
) %>% 
  select(model, r.squared, AIC, BIC)

model_comparison
```
* R-squared: mod06 is best with the highest value of 0.7819. This means it explains the most variance in y. However, R-squared always increases with more features, so this metric alone favors the most complex model.
* AIC: mod03 is best with the lowest AIC of 223.6973. AIC penalizes model complexity, and mod03 strikes the best balance between fit and complexity.
* BIC: mod03 is also best with the lowest BIC of 249.7490. BIC applies a stronger penalty for complexity than AIC, and again mod03 wins.
* Overall, mod03 is the best model according to the penalized metrics AIC and BIC. This suggests that the quadratic interaction model provides the best trade-off between fit and complexity without overfitting.
...
What do you think?  
* The predictions are consistent with the AIC/BIC rankings. mod03 produces smooth trends that capture the inverted U-shape in x2 with reasonable uncertainty intervals, confirming it as the best model.
* The defining characteristic of the worst models (mod04, mod05, mod06) is extreme instability at the boundaries of x1. The mean trend oscillates wildly and prediction intervals explode. This is a clear sign of overfitting due to too many parameters relative to the available data.
...
X03 <- model.matrix(mod03, data = df)
...
X06 <- model.matrix(mod06, data = df)
...
lm_logpost <- function(unknowns, my_info)
{
  ...
  # sum together
  return (log_prior + log_lik + log_derive_adjust)
}
```
...
```{r, solution_03c}
### add more code chunks if you like
start_03 <- rep(0, ncol(X03) + 1)
start_06 <- rep(0, ncol(X06) + 1)

laplace_03_weak <- my_laplace(start_03, lm_logpost, info_03_weak)
laplace_06_weak <- my_laplace(start_06, lm_logpost, info_06_weak)
...
viz_post_coefs(
  ...
  post_sds = sqrt(diag(laplace_03_weak$var_matrix)[1:ncol(X03)]),
  xnames = colnames(X03)
)
```
...
viz_post_coefs(
  ...
  post_sds = sqrt(diag(laplace_06_weak$var_matrix)[1:ncol(X06)]),
  xnames = colnames(X06)
)
```
...
```{r, solution_03e}
### add more code chunks if you like
log_bf <- laplace_03_weak$log_evidence - laplace_06_weak$log_evidence
...
bf
```
Since the Bayes Factor is highly greater than 1, there is evidence in favor of model 3 over model 6. A Bayes Factor this large indicates that model 3 is much more probable than model 6 given the data. This confirms that the simpler quadratic interaction model is strongly preferred over the complex spline model under the weak prior.
...
```{r, solution_03f_b}
### add more code chunks if you like
laplace_03_strong <- my_laplace(start_03, lm_logpost, info_03_strong)
laplace_06_strong <- my_laplace(start_06, lm_logpost, info_06_strong)
...
viz_post_coefs(
  ...
  post_sds = sqrt(diag(laplace_03_strong$var_matrix)[1:ncol(X03)]),
  xnames = colnames(X03)
)
```
...
viz_post_coefs(
  ...
  post_sds = sqrt(diag(laplace_06_strong$var_matrix)[1:ncol(X06)]),
  xnames = colnames(X06)
)
```
...
```{r, solution_03h_b}
### add more code chunks if you like
laplace_03_very_strong <- my_laplace(start_03, lm_logpost, info_03_very_strong)
laplace_06_very_strong <- my_laplace(start_06, lm_logpost, info_06_very_strong)
...
viz_post_coefs(
  ...
  post_sds = sqrt(diag(laplace_03_very_strong$var_matrix)[1:ncol(X03)]),
  xnames = colnames(X03)
)
```
```{r, solution_03i_b}
viz_post_coefs(
  ...
  post_sds = sqrt(diag(laplace_06_very_strong$var_matrix)[1:ncol(X06)]),
  xnames = colnames(X06)
)
```
...
What do you think?  
As the prior standard deviation decreased from 50 to 1 to 1/50, the coefficient posteriors became increasingly shrunk toward zero. With the weak prior, the data drives the estimates. I(x2^2) is clearly negative and the intercept notably positive. As the prior strengthens, all coefficients are pulled toward zero regardless of the data, with the very strong prior compressing everything to near zero (x-axis scale shrinks from ±0.5 to ±0.075). This pattern is consistent across both models, which confirms that a stronger prior acts as stronger regularization.
...
```{r, solution_03k}
# Strong prior Bayes Factor
log_bf_strong <- laplace_03_strong$log_evidence - laplace_06_strong$log_evidence
bf_strong <- exp(log_bf_strong)

# Very strong prior Bayes Factor
log_bf_very_strong <- laplace_03_very_strong$log_evidence - laplace_06_very_strong$log_evidence
bf_very_strong <- exp(log_bf_very_strong)
...
bf_very_strong
```
Yes, the prior does influence which model is considered better. Under the weak and strong priors, model 3 is preferred. But, under the very strong prior, model 6 is preferred. This makes sense because the very strong prior suppresses all coefficients toward zero regardless of the data. With so many coefficients being shrunk, the more complex model 6 benefits more from this extreme regularization. This shifts the Bayes Factor in its favor.
...
X06_test <- model.matrix(~ splines::ns(x1, df = 12, 
                          knots = x1_knots$interior_knots,
                          Boundary.knots = x1_knots$boundary_knots) * 
...
What do you think?  
* As the prior standard deviation decreased, predictions became increasingly regularized toward zero. For model 3, the weak prior captures a slight inverted U-shape with wide intervals, while the very strong prior compresses the mean trend nearly to zero with tight confidence intervals.
* For model 6, the effect is more dramatic. The weak prior produces highly erratic predictions at the boundaries, but the very strong prior tames this instability considerably, which pushed predictions close to zero.
* These results are consistent with the posterior coefficients. Stronger priors shrink coefficients toward zero, collapsing the predicted mean trends accordingly.
...
```{r, solution_05a}
X03_glmnet <- model.matrix(~ (x1 + I(x1^2)) * (x2 + I(x2^2)) - 1, data = df)
X06_glmnet <- model.matrix(~ splines::ns(x1, df = 12) * (x2 + I(x2^2) + I(x2^3) + I(x2^4)) - 1, data = df)
```
...
```
* Model 3: At lambda.min (left dotted line), 8 coefficients are remaining after tuning. This matches the total number of features in the model (the Lasso did not eliminate any features).
* Model 6: At lambda.min (left dotted line), 2 coefficients are remaining after tuning. This means the Lasso eliminated almost all 64 features, keeping only 2 non-zero coefficients.
...
```
* Model 3 (mod03): All 8 features remain active: x1, I(x1^2), x2, I(x2^2), x1:x2, x1:I(x2^2), I(x1^2):x2, and I(x1^2):I(x2^2). The Lasso did not zero out any features for model 3.
* Model 6 (mod06): Only 3 features remain active: the intercept, I(x2^2) (-0.387), and splines::ns(x1, df=12)12:x2 (-0.109). All other features have been turned off by the Lasso penalty.
* Are the final tuned models different? Yes, they are very different. Model 3 retains all its features while model 6 is penalized, keeping only 2 non-intercept features. But, both models converge on I(x2^2) as an important feature. This is consistent with our earlier findings that the quadratic effect of x2 is the dominant signal in the data.
